...
export default class Slider extends React.Component<Partial<SlideProps>> {
    ...
    handleDragMouseUp = () => {
        if (this.props.onAfterChange && typeof this.tempR === 'number') {
            this.props.onAfterChange(this.tempR);
        }
        this.clearTemp();
    ...
    handleTrackMouseDown = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        ...
        if (this.trackElem) {
            ...
            let offset_temp_p = (clientX - this.trackElem.getBoundingClientRect().left) * 100 / offsetWidth;
            if (this.props.layout === 'vertical')
                offset_temp_p = (this.trackElem.getBoundingClientRect().bottom - clientY) * 100 / offsetHeight;
